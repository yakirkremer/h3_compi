...
bool SymbolTable::add_symbol(string name, string type,int size, bool is_function = false, string arg_type = "") {
    ...
    symbols.push_back(new Symbol(name, type, size, is_function,arg_type));
    if (size >= 0)
        max_offset = size;
    this->offset+=size;
    return true;
...
bool Scopes::add_symbol(const string &name, const string &type, int offset, string arg_type,bool is_function) {
    if(scope->symbol_exists(name)){
        return false;
    }
    scope->add_symbol(name, type, offset, is_function, arg_type);
    return true;
}
